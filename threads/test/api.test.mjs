...
test("실제 posts.md: 길이 제한 · 일차 배분 · 오퍼 글 위치", () => {
  ...
  // 0 으로 시작하는 수는 가격이 아니다. "제 실적은 0원입니다" 같은 문장을
  // 가격으로 잡으면 안 된다 — 그건 오히려 1일차에 있어야 하는 문장이다.
  const PRICE = /(?<![\d,])[1-9][\d,]*\s*원|만원/;
  assert.ok(
  ...
  assert.match(
    posts[0].text,
    /실적은 (없습니다|0원)/,
    "이 계정의 판매 실적이 아직 없다는 걸 먼저 밝혀야 한다",
  );
  assert.match(
    posts[0].text,
    /통신판매업|스마트스토어|공동구매/,
  ...
  // 글을 쓰는 건 AI 다. 사장의 경력을 1인칭으로 쓰면 그 순간 거짓말이 되고,
  // 이 계정이 가진 유일한 신뢰 근거가 같이 무너진다. 주어를 붙여둬야 한다.
  assert.match(posts[0].text, /사장은/, "사장의 경력은 주어를 밝혀야 한다");
  assert.doesNotMatch(
    posts[0].text,
    /(제가|저는)[^.\n]*(공동구매|통신판매업 신고는 2021)/,
    "사장의 경력을 AI 가 1인칭으로 말하면 안 된다",
  );
